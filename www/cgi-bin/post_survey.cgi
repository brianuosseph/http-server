...
if environ.has_key("HTTP_COOKIE"):
  ...
  for cookie in environ["HTTP_COOKIE"].split(";"):
    ...
    if key == "post_survey_correct_answers":
      num_correct_answers = int(value)
    elif key == "post_survey_questions_answered":
...
if form.getvalue('submission') and form.getvalue('answer'):
  ...
  or (int(submitted_for_question) == 5 and sumbitted_answer == "There") :
    print "Set-Cookie: post_survey_correct_answers=%s" % str(num_correct_answers + 1)
  print "Set-Cookie: post_survey_questions_answered=%s" % str(num_questions_answered + 1)
  print
  ...
  if int(submitted_for_question) == 5:
    print """
      <html>
        <head>
          <meta http-equiv="REFRESH" content="0;url=/cgi-bin/post_survey.cgi">
        </head>
      </html>
      """
  # Else redirect to next question
    ...
    print """
      <html>
        <head>
          <meta http-equiv="REFRESH" content="0;url=/cgi-bin/post_survey.cgi?question=%s">
        </head>
      </html>
...
# Else just print the question
  ...
  html_header = """
    <html>
      ...
      <body>
        <header class="header">
          <h1>CGI Survey</h1>
          <a href="/cgi-bin/post_survey.cgi" id="survey-quit">X</a>
        </header>
  ...
  and 0 < int(form.getvalue('question')) < 6:
    ...
    if current_question == 1:
      ...
      print """
        ...
        <form action="/cgi-bin/post_survey.cgi" method="POST">
  ...
  # Survey start or completion page
    ...
    print "Set-Cookie: post_survey_questions_answered=0"
    print "Set-Cookie: post_survey_correct_answers=0"
    print
    print html_header
